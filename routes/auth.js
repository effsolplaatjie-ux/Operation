...
router.post('/register', async (req, res) => {
    try {
        ...
        // Residents auto-approve. CPF/SAPS/Orgs go to pending.
        ...
        await pool.query(sql, [
            role, status, username, hash, full_name, email || null, phone || null,
            house_no || null, street || null, suburb || null, city || null, 
            municipality || null, district_municipality || null, province || null, 
            country || 'South Africa', station_name || null, area_of_operation || null, 
            contact_person_name || null, contact_person_details || null
        ]);
...
router.post('/login', async (req, res) => {
    ...
    // Hardcoded Admin (Reminder: This account CANNOT post alerts because ID 0 does not exist in the DB)
    ...
    try {
        ...
        res.json({ token, role: users[0].role, userId: users[0].id }); // This sends the REAL database ID!
    } catch (err) {
